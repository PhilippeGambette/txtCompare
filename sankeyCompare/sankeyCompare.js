...
/***************************************************************************************************/
/* The following lines were coded by Gemini 3 rapide: https://gemini.google.com/share/de6d72a30ed4 */
/***************************************************************************************************/
async function chargerEtTraiterCSV(url1, url2) {
  try {
    // 1. Chargement des deux fichiers en parallèle
    const [res1, res2] = await Promise.all([
      chargerCSV(url1),
      chargerCSV(url2)
    ]);

    const lignesUrl1 = res1.data;
    const lignesUrl2 = res2.data;

    // Initialisation de l'objet final
    const customData = {};

    // 2. Traitement de URL2 (Propriétés dynamiques basées sur la ligne 1 et ligne 2)
    if (lignesUrl2.length >= 2) {
      const cles = lignesUrl2[0];   // Première ligne (les en-têtes)
      const valeurs = lignesUrl2[1]; // Deuxième ligne (les valeurs juste en-dessous)

      cles.forEach((cle, index) => {
        if (!cle) return; // Ignore les colonnes vides sans titre

        let valeur = valeurs[index] || "";

        // Conversion en entier pour "ecart" et "pixelsParPage"
        if (cle === "ecart" || cle === "pixelsParPage") {
          valeur = parseInt(valeur, 10) || 0; // 0 par défaut si la conversion échoue
        }

        customData[cle] = valeur;
      });
    }

    // 3. Traitement de URL1 (Tableau de tableaux de chaînes de caractères)
    // Papa Parse nous donne déjà un tableau de tableaux de chaînes par défaut
    customData.texts = lignesUrl1;

    return customData;

  } catch (erreur) {
    console.error("Erreur lors du traitement des CSV :", erreur);
  }
}

// Fonction utilitaire pour "promisifier" Papa.parse
function chargerCSV(url) {
  return new Promise((resolve, reject) => {
    Papa.parse(url, {
      download: true,
      skipEmptyLines: true, // Évite les lignes vides parasites à la fin du fichier
...
let urlFichier1 = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQgInowD87JuydnN8tx2rb9LuQ2JojYAX6oRWjw8ovlA8UstmNI6ZM1xv5I7ftT9Doc2NiFrAzkP-b6/pub?gid=0&single=true&output=csv";
let urlFichier2 = "https://docs.google.com/spreadsheets/d/e/2PACX-1vQgInowD87JuydnN8tx2rb9LuQ2JojYAX6oRWjw8ovlA8UstmNI6ZM1xv5I7ftT9Doc2NiFrAzkP-b6/pub?gid=668224341&single=true&output=csv";
if (params.get("data") != null){
   // ?data=https://docs.google.com/spreadsheets/d/e/2PACX-1vQgInowD87JuydnN8tx2rb9LuQ2JojYAX6oRWjw8ovlA8UstmNI6ZM1xv5I7ftT9Doc2NiFrAzkP-b6/pub¤gid=0¤¤single=true¤¤output=csv&metadata=https://docs.google.com/spreadsheets/d/e/2PACX-1vQgInowD87JuydnN8tx2rb9LuQ2JojYAX6oRWjw8ovlA8UstmNI6ZM1xv5I7ftT9Doc2NiFrAzkP-b6/pub¤gid=668224341¤¤single=true¤¤output=csv
   urlFichier1 = params.get("data").replaceAll("¤¤","&").replaceAll("¤","?");
   urlFichier2 = params.get("metadata").replaceAll("¤¤","&").replaceAll("¤","?");
}


chargerEtTraiterCSV(urlFichier1, urlFichier2).then((resultat) => {
  console.log("Objet data final :", resultat);
  data.push(resultat);

    /*****************************************************/
    /* Code below written by Philippe Gambette witout AI */
    /*****************************************************/
    ...
    $("#choix").on("change",event=>{window.history.pushState({site: 0}, '', "?id="+$("#choix").val());loadSankeyCompare()});
...
$("#customUrl").on("click",function(){
   let url = "http://philippegambette.github.io/txtCompare/sankeyCompare/?id=5&data="+$("#dataCsv").val().replaceAll("&","¤¤").replaceAll("?","¤")+"&metadata="+$("#metadataCsv").val().replaceAll("&","¤¤").replaceAll("?","¤");
